...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    for(int is=0;is<(int)inputStrings.size();is++)
    {
        ...
        if (address.IsValid()) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Hdac address or script");	// HDAC
        }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Hdac address");	// HDAC
    CKeyID keyID;
